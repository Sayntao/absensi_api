<template>
  ...
  >
    <!-- Tombol X Close menggunakan $emit('close') -->
    ...
    <div class="px-2 pr-14">
      <h4 class="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">Tambah Shift</h4>

      <p class="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
        Isi detail shift yang baru.
      </p>
    </div>

    <VeeForm :validation-schema="schema" @submit="onSubmit" class="flex flex-col">
      <div class="px-2 overflow-y-auto custom-scrollbar">
        <!-- FIELD 1: Shift Name (shift_name) -->
        ...
        <!-- FIELD 2: Check-in Time (check_in_time) -->
        ...
        <!-- FIELD 3: Check-out Time (check_out_time) -->
...
const emit = defineEmits(['close', 'shiftAdded'])
...
const postShift = async (formData, resetForm) => {
  loading.value = true
  error.value = null
  try {
    const response = await api.post('shift', formData)
    console.log('API Response Sukses:', response.data)
    console.log('Data shift berhasil disimpan!', formData)
    emit('shiftAdded')
    resetForm()
    emit('close')
  } catch (err) {
    console.error('Error saat membuat Shift:', err)
    if (err.response) {
